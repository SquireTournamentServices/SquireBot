...
commandSnippets[
    "setup"
] = "- setup : Creates a tournament and has a toggle to enable tricebot."
...
async def setupGuild(ctx):
    ...
    if await isPrivateMessage(ctx):
        return
    gld = guildSettingsObjects[ctx.guild.id]

    if not gld.isGuildAdmin(ctx.author):
        await ctx.send(
            f"{mention}, you are not an admin on this server, so you can not run this command."
        )
        return

    if gld.isConfigured():
        await ctx.send(
            f"{mention}, {ctx.guild.name} is already setup to run tournaments. If you want to change what channels are used, either use the !update-properties or !update-server-defaults commands. Instructions those commands can be found be using the !squirebot-help command followed by the command name."
        )
        return
    ...
    await ctx.send(
        content=f"{mention}, below are the channels/categories that are going to be created. They can be moved around as desired. Are you sure you want to finish setting up (!yes/!no)?",
        embed=embed,
    )
...
commandSnippets[
    "create-tournament"
] = "- create-tournament : Creates a tournament and has a toggle to enable tricebot."
...
async def createTournament(ctx, tournName=None, tournType=None, *args):
    ...
    if len(tournProps) != "".join(args).count("="):
        await ctx.send(
            f'{mention}, there is an issue with the tournament properties that you gave. Check that you entered them correctly and consult the "!squirebot-help" command for more help'
        )
        return
    ...
    if tournName is None or tournType is None:
        await ctx.send(
            f"{mention}, you need to specify what you want the tournament name and type."
        )
        return
    ...
    if not (shouldBeNone is None):
        await ctx.send(
            f"{mention}, there is already a tournament call {tournName} on this server. Pick a different name."
        )
        return
    ...
    try:
        ...
        newLine = "\n\t- "
        await ctx.send(
            f"{mention}, invalid tournament type of {tournType}. The supported tournament types are:{newLine}{newLine.join(tournamentTypes)}."
        )
        return
    ...
    await ctx.send(
        f"{adminMention}, a new tournament called {tournName} was created{trice} by {mention}:\n{message}"
    )
...
commandSnippets[
    "update-properties"
] = "- update-properties : Changes the properties of a tournament."
...
async def updateTournProperties(ctx, tournName=None, *args):
    ...
    if tournName is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament and a number of players for a match."
        )
        return
    ...
    if tournObj is None:
        await ctx.send(
            f"{mention}, there is not a tournament called {tournName!r} on this server."
        )
        return
    ...
    if len(tournProps) != "".join(args).count("="):
        print(tournProps)
        await ctx.send(
            f'{mention}, there is an issue with the tournament properties that you gave. Check your spelling and consult the "!squirebot-help" command for more help'
        )
        return
    ...
    await ctx.send(
        f"{adminMention}, {mention} has updated the properties of {tournName}.\n{message}"
    )
...
] = "- update-server-defaults : Changes the properties of a tournament."
...
async def updateTournProperties(ctx, *args):
    ...
    if not gld.isGuildAdmin(ctx.author):
        await ctx.send(
            f"{mention}, you do not have permissions to change the tournament settings on this server."
        )
        return
    ...
    gld.saveSettings()
    await ctx.send(
        f"{mention}, you have updated the tournament settings for the server.\n{message}"
    )


commandSnippets[
    "tricebot-status"
] = "- tricebot-status : Displays the status and settings of tricebot for a tournament"
...
async def triceBotStatus(ctx, tourn=None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you need to specify what tournament you want to know the settings for."
        )
        return
    ...
    if tournObj is None:
        await ctx.send(
            f"{mention}, there is not a tournament called {tourn!r} on this server."
        )
        return
    ...
    if tournObj.triceBotEnabled:
        ...
        await ctx.send(
            f"{adminMention}, tricebot is enabled for {tourn!r} and has the following settings:\n```{settings_str}```"
        )
    else:
...
async def updateReg(ctx, tourn=None, status=None):
    ...
    if tourn is None or status is None:
        await ctx.send(
            f'{mention}, it appears that you did not give enough information. You need to first state the tournament name and then "true" or "false".'
        )
        return
    ...
    if tournObj is None:
        await ctx.send(
            f"{mention}, there is not tournament called {tourn!r} on this server."
        )
        return
    ...
    tournObj.saveOverview()
    await ctx.send(
        f'{adminMention}, registration for the {tourn!r} tournament has been {("opened" if str_to_bool(status) else "closed")} by {mention}.'
    )
    await tournObj.updateInfoMessage()
...
commandSnippets[
    "start-tournament"
] = "- start-tournament : Starts the tournament, which closes registration and let's players LFG"
...
async def startTournament(ctx, tourn=None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you need to specify what tournament you want to start."
        )
        return
...
async def endTournament(ctx, tourn: str = None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you need to specify what tournament you want to end."
        )
        return
    ...
    commandsToConfirm[ctx.author.id] = (
        getTime(),
        30,
        gld.endTournament(tourn, mention),
    )
    await ctx.send(
        f"{adminMention}, in order to end {tourn}, confirmation is needed. {mention}, are you sure you want to end {tourn} (!yes/!no)?"
    )
    await tournObj.updateInfoMessage()
...
commandSnippets[
    "prune-decks"
] = "- prune-decks : Removes decks from players until they have the max number"
...
async def adminPruneDecks(ctx, tourn=None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament."
        )
        return
    ...
    await ctx.send(
        f"{adminMention}, in order to prune decks, confirmation is needed. {mention}, are you sure you want to prune decks (!yes/!no)?"
    )


commandSnippets[
    "prune-players"
] = "- prune-players : Drops players that didn't submit a deck"
...
async def adminPruneDecks(ctx, tourn=None):
    ...
    await ctx.send(
        f"{adminMention}, in order to prune players, confirmation is needed. {mention}, are you sure you want to prune players (!yes/!no)?"
    )
...
async def adminCreatePairing(ctx, tourn=None, *plyrs):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament, match number, player, and result in order to remove a player from a match."
        )
        return
    ...
    if len(plyrs) != tournObj.playersPerMatch:
        await ctx.send(
            f"{mention}, {tourn} requires {tournObj.playersPerMatch} be in a match, but you specified {len(plyrs)} players."
        )
        return
    ...
    tournObj.saveOverview()
    await ctx.send(
        f"{mention}, the players you specified for the match are now paired. Their match number is #{tournObj.matchReg.getMatchCount()}."
    )


commandSnippets[
    "pair-round"
] = "- pair-round : Creates the pairings for the next round."
...
async def pairRound(ctx, tourn=None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament to pair it for its next round."
        )
        return
    ...
    if (
        len(tournObj.matchReg.getUncertifiedMatches())
        + len(tournObj.matchReg.getOpenMatches())
        != 0
    ):
        await ctx.send(
            f"{mention}, there are matches that have not finished yet. All results for the round must confirmed before the next round can begin."
        )
        return
    ...
    commandsToConfirm[ctx.author.id] = (
        getTime(),
        30,
        tournObj.confirmPairings(mention),
    )
    await ctx.send(
        f"{adminMention}, in order to pair the next round, confirmation is needed. {mention}, are you sure you want to start the next round (!yes/!no)?"
    )
...
] = "- create-pairings-list : Creates a list of possible match pairings (unweighted)"
...
async def createPairingsList(ctx, tourn=None):
    ...
    if not queue.readyToPair(tournObj.playersPerMatch):
        await ctx.send(
            f"{mention}, there are not enough players in {tourn} to create a match."
        )
        return
    ...
    if queue.size() == 0:
        await ctx.send(
            f"{mention}, here is a list of possible pairings. No players are left unmatched."
        )
    else:
    ...
    queueStr = [
        [f"{tournObj.getPlayer(plyr).getMention()}" for plyr in pairing]
        for pairing in pairings
    ]
...
] = "- set-pairing-threshold : Sets the number of players needed to pair the queue"
...
async def pairingsThreshold(ctx, tourn=None, num=None):
    ...
    try:
        ...
        await ctx.send(
            f"{mention}, {num!r} could not be converted to a number. Please make sure you only use digits."
        )
        return
    ...
    tournObj.saveOverview()
    await ctx.send(
        f"{adminMention}, the pairings threshold for {tourn} was changed to {num} by {mention}."
    )
...
async def adminDropPlayer(ctx, tourn=None, plyr=None):
    ...
    if tourn is None or plyr is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament and a player."
        )
        return
    ...
    commandsToConfirm[ctx.author.id] = (
        getTime(),
        30,
        tournObj.dropPlayerAdmin(plyr, mention),
    )
    await ctx.send(
        f"{adminMention}, in order to drop {plyr}, confirmation is needed. {mention}, are you sure you want to drop this player (!yes/!no)?"
    )
...
async def adminRemoveMatch(ctx, tourn=None, mtch=None):
    ...
    try:
        ...
        await ctx.send(
            f"{mention}, you did not provide a match number. Please specify a match number using digits."
        )
        return
    ...
    commandsToConfirm[ctx.author.id] = (
        getTime(),
        30,
        tournObj.removeMatch(mtch, mention),
    )
    await ctx.send(
        f"{adminMention}, in order to remove match #{mtch}, confirmation is needed. {mention}, are you sure you want to remove this match (!yes/!no)?"
    )


commandSnippets[
    "tournament-status"
] = "- tournament-status : Prints the currect matchmaking queue"
...
async def viewQueue(ctx, tourn=None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament to view the queue."
        )
        return
...
commandSnippets[
    "tricebot-kick-player"
] = "- tricebot-kick-player : Kicks a player from a cockatrice match when tricebot is enabled for that match"
...
async def tricebotKickPlayer(ctx, tourn=None, mtch=None, playerName=None):
    ...
    if result == 1:
        ...
        await ctx.send(
            f"{mention}, An error has occured whilst kicking {playerName!r} from match {mtch}."
        )


commandSnippets[
    "tricebot-disable-pdi"
] = "- tricebot-disable-pdi : Disables player deck verification."
...
async def triceBotUpdatePlayer(ctx, tourn=None, mtch=None):
    ...
    if not Match.playerDeckVerification:
        await ctx.send(
            f"{mention}, that match is not a match with player deck verification enabled."
        )
        return
...
commandSnippets[
    "tricebot-update-player"
] = "- tricebot-update-player : Updates the cockatrice username for a player, for a game that is ongoing."
commandCategories["day-of"].append("tricebot-update-player")


@bot.command(name="tricebot-update-player")
async def triceBotUpdatePlayer(
    ctx, tourn=None, mtch=None, plyr=None, newTriceName=None
):
    mention = ctx.author.mention
    gld = guildSettingsObjects[ctx.guild.id]
    ...
    if tourn is None or mtch is None or plyr is None or newTriceName is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament, a match, a player, and their new name."
        )
        return
    ...
    if member is None:
        await ctx.send(
            f"{mention}, there is not a member of this server by {plyr!r}, assuming this is the problematic cockatrice name."
        )
        ...
        if member.id in tournObj.players:
            ...
            await ctx.send(
                f"{mention}, a player by {plyr!r} was found, but they have not registered for {tourn}. Make sure they register first."
            )
            return
    ...
    if result == 0:
        ...
        await ctx.send(
            f"{mention}, the player information was successfully updated, however a player using that player's name is in the game."
        )
        if not member is None:
            tournObj.setPlayerTriceName(member.id, newTriceName)  # Update trice name
    elif result == -1:
        await ctx.send(
            f"{mention}, the game was not found, so the player information was not updated, as there no action was taken."
        )
    elif result == -2:
        await ctx.send(
            f"{mention}, the player was not found, so no action was taken. If there are multiple players with no cockatrice names then you can ignore this error as they are still able to join."
        )
    else:
...
commandSnippets[
    "download-replays"
] = "- download-replays : Downloads all replays for a tournament"
...
async def downloadReplays(ctx, tourn=None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament to download the replays for."
        )
        return
    ...
    await ctx.send(
        f"{mention}, here are the replays for {tourn}.\n{message}",
        file=discord.File(replayFile, f"{tourn}- replays.zip"),
    )
    replayFile.close()
...
commandSnippets[
    "raw-standings"
] = "- raw-standings : Creates a text file with standings for Mike."
...
async def rawStandings(ctx, tourn=None):
    ...
    if tourn is None:
        ...
        if len(tourns) > 1:
            await ctx.send(
                f"{mention}, there are multiple tournaments planned in this server. Please specify which tournament you would like to see the standings of."
            )
            return
        elif len(tourns) < 1:
            await ctx.send(
                f"{mention}, there are no planned tournaments for this server. If you think this is an error, contact tournament staff."
            )
            return
        else:
            tournObj = tourns[0]
        ...
        if tournObj is None:
            await ctx.send(
                f'{mention}, there is not a tournament called {tourn!r} in this server".'
            )
            return
    ...
    with open("standings.txt", mode="w+") as attachment:
        attachment.write(
            "Placement, Players, Match Points, Win Percentage, Opponent WP\n"
        )
        length = len(standings[0])
        for i in range(length):
            attachment.write(
                f'{standings[0][i]}, {standings[1][i].name.replace(",", "")}, {standings[2][i]}, {trunk(standings[3][i])}, {trunk(standings[4][i])}\n'
            )

    with open("standings.txt", mode="r") as attachment:
        await ctx.send(
            content=f"{mention}, the standings for {tourn} are in the attached file.",
            file=discord.File(attachment, "standings.txt"),
        )
...
async def cutTopXCoroFunc(ctx, mention, standings, tournObj, tourn, x):
    ...
    newLine = "\n\t- "
    digest.setContent(
        f"{mention}, {tourn} was cut to the top {x} players, the following players were dropped:{newLine}{newLine.join(playersDropped)}"
    )
    return digest
...
async def cutToTopX(ctx, tourn=None, x=None):
    ...
    if tourn is None or x is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament and the number of players to cut to."
        )
        return
    ...
    try:
        ...
        await ctx.send(
            f"{mention}, you must insert a whole number for the amount of players to cut to."
        )
        return
    if x < 2:
    ...
    if x > len(standings[1]):
        await ctx.send(
            f"{mention}, there are not enough players with standings to make this cut."
        )
        return
    ...
    commandsToConfirm[ctx.author.id] = (
        getTime(),
        30,
        cutTopXCoroFunc(ctx, mention, standings, tournObj, tourn, x),
    )
    await ctx.send(
        f"{adminMention}, in order to cut players to the top {x}, confirmation is needed. {mention}, are you sure you want to remove this match (!yes/!no)?"
    )
