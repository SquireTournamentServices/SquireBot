...
LINK_TO_PLAYER_CMD_DOC = (
    "https://gitlab.com/monarch3/SquireBot/-/tree/development/docs/UserCommands.md"
)
LINK_TO_JUDGE_CMD_DOC = (
    "https://gitlab.com/monarch3/SquireBot/-/tree/development/docs/JudgeCommands.md"
)
LINK_TO_ADMIN_CMD_DOC = (
    "https://gitlab.com/monarch3/SquireBot/-/tree/development/docs/AdminCommands.md"
)
LINK_TO_CRASH_COURSE_DOC = (
    "https://gitlab.com/monarch3/SquireBot/-/tree/development/docs/CrashCourse.md"
)
...
async def sendAdminHelpMessage(ctx) -> None:
    ...
    embed.add_field(
        name="**Registration**",
        value="\n".join(
            [commandSnippets[cmd] for cmd in commandCategories["registration"]]
        ),
        inline=False,
    )
    embed.add_field(
        name="**Match**",
        value="\n".join([commandSnippets[cmd] for cmd in commandCategories["playing"]]),
        inline=False,
    )
    embed.add_field(
        name="**Miscellaneous**",
        value="\n".join([commandSnippets[cmd] for cmd in commandCategories["misc"]]),
        inline=False,
    )
    ...
    embed.add_field(
        name="**Registration**",
        value="\n".join(
            [commandSnippets[cmd] for cmd in commandCategories["admin-registration"]]
        ),
        inline=False,
    )
    embed.add_field(
        name="**Match**",
        value="\n".join(
            [commandSnippets[cmd] for cmd in commandCategories["admin-playing"]]
        ),
        inline=False,
    )
    embed.add_field(
        name="**Miscellaneous**",
        value="\n".join(
            [commandSnippets[cmd] for cmd in commandCategories["admin-misc"]]
        ),
        inline=False,
    )
    ...
    embed.add_field(
        ...
        value="\n".join(
            [commandSnippets[cmd] for cmd in commandCategories["management"]]
        ),
        inline=False,
    )
    embed.add_field(
        name="**Properties**",
        value="\n".join(
            [commandSnippets[cmd] for cmd in commandCategories["properties"]]
        ),
        inline=False,
    )
    embed.add_field(
        name="**Day-Of**",
        value="\n".join([commandSnippets[cmd] for cmd in commandCategories["day-of"]]),
        inline=False,
    )

    embed.add_field(
        name="**__Additional Information__**",
        value=f"The full documentation for the judge commands can be found [here]({LINK_TO_JUDGE_CMD_DOC}) and admin commands [here]({LINK_TO_ADMIN_CMD_DOC}). The user commands are [here]({LINK_TO_PLAYER_CMD_DOC}), and the crash course is [here]({LINK_TO_PLAYER_CMD_DOC}). If you have ideas about how to improve this bot, [let us know](https://forms.gle/jt9Hpaz3ZcVNfeiRA)!",
        inline=False,
    )
...
async def sendJudgeHelpMessage(ctx) -> None:
    ...
    embed.add_field(
        name="**__Additional Information__**",
        value=f"The full documentation for the judge commands can be found [here]({LINK_TO_JUDGE_CMD_DOC}). The user commands are [here]({LINK_TO_PLAYER_CMD_DOC}), and the crash course is [here]({LINK_TO_CRASH_COURSE_DOC}). If you have ideas about how to improve this bot, [let us know](https://forms.gle/jt9Hpaz3ZcVNfeiRA)!",
        inline=False,
    )
...
async def sendUserHelpMessage(ctx) -> None:
    ...
    embed.add_field(
        name="**__Registration Commands__**",
        value="\n".join(
            [commandSnippets[cmd] for cmd in commandCategories["registration"]]
        ),
        inline=False,
    ...
    embed.add_field(
        name="**__Match Commands__**",
        value="\n".join([commandSnippets[cmd] for cmd in commandCategories["playing"]]),
        inline=False,
    )
    embed.add_field(
        name="**__Miscellaneous Commands__**",
        value="\n".join([commandSnippets[cmd] for cmd in commandCategories["misc"]]),
        inline=False,
    )
    ...
    embed.add_field(
        name="**__Additional Information__**",
        value=f"Additional information about each command can be found [here]({LINK_TO_PLAYER_CMD_DOC}). There is also a [crash course]({LINK_TO_CRASH_COURSE_DOC}) for new users. If you have ideas about how to improve this bot, [let us know](https://forms.gle/jt9Hpaz3ZcVNfeiRA)!",
        inline=False,
    )
...
async def isPrivateMessage(ctx, send: bool = True) -> bool:
    ...
    if digest and send:
        await ctx.send(
            f"You are not allowed to send most commands via DM. To see what commands you can send via DM, use the !squirebot-help command."
        )
    return digest
...
async def isAdmin(ctx, send: bool = True) -> bool:
    ...
    if not digest and send:
        await ctx.send(
            f"{ctx.author.mention}, invalid permissions: You are not tournament staff. Please do not use this command again or {adminMention} or {judgeMention} may intervene."
        )
    return digest
...
async def isTournamentAdmin(ctx, send: bool = True) -> bool:
    ...
    if not digest and send:
        await ctx.send(
            f"{ctx.author.mention}, invalid permissions: You are not tournament staff. Please do not use this command again or {adminMention} may intervene."
        )
    return digest
...
async def isTournDead(tourn, ctx, send: bool = True) -> bool:
    ...
    if digest and send:
        await ctx.send(
            f"{ctx.author.mention}, {tourn} has ended or been cancelled. Contact {tourn.tournAdminRole} if you think this is an error."
        )
    return digest
...
async def isRegOpen(tourn, ctx, send: bool = True) -> bool:
    ...
    if send and not digest:
        await ctx.send(
            f"{ctx.author.mention}, registration for {tourn} is closed. Please contact tournament staff if you think this is an error."
        )
    return digest
...
async def hasRegistered(tourn, plyr, ctx, send: bool = True) -> bool:
    ...
    if send and not digest:
        await ctx.send(
            f"{ctx.author.mention}, you are not registered for {tourn.name}. Please register before trying to access the tournament."
        )
    return digest
...
async def isActivePlayer(tourn, plyr, ctx, send: bool = True) -> bool:
    ...
    if send and not digest:
        await ctx.send(
            f"{ctx.author.mention}, you registered for {tourn.name} but have been dropped. Contact tournament staff if you think this is an error."
        )
    return digest
...
async def hasOpenMatch(tourn, plyr, ctx, send: bool = True) -> bool:
    ...
    if send and not digest:
        await ctx.send(
            f"{ctx.author.mention}, you are not an active player in a match. You do not need to do anything."
        )
    return digest
...
async def hasCommandWaiting(ctx, user: int, send: bool = True) -> bool:
    ...
    if send and digest:
        await ctx.send(
            f"{ctx.author.mention}, you have a command waiting for your confirmation. That confirmation request is being overwriten by this one."
        )
    return digest
...
async def createMisfortune(ctx) -> None:
    ...
    if playerMatch is None:
        await ctx.send(
            f"{ctx.author.mention}, you are not in an open match, so you can not create any misfortune."
        )
        return

    await ctx.send(
        f"{ctx.author.mention}, you have created misfortune for {playerMatch.getMention()}. How will you all respond? (send via DM)"
    )
    for plyr in playerMatch.activePlayers:
        await tournself.getPlayer(plyr).sendMessage(
            content=f'Misfortune has been created in your match. Tell me how you will respond (with "!misfortune [number]")!'
        )
...
async def recordMisfortune(ctx, misfortune, num: int) -> bool:
    ...
    await ctx.send(
        f"{ctx.author.mention}, your response to this misfortune has been recorded!"
    )
    if len(misfortune[1].misfortunes) == len(misfortune[1].activePlayers):
        ...
        for tourn in tourns.values():
            ...
            if tourn.players[ctx.author.id].hasOpenMatch():
                break
        newLine = "\n\t"
        printout = newLine.join(
            [
                f"{tournself.getPlayer(plyr).getMention()}: {misfortune[1].misfortunes[plyr]}"
                for plyr in misfortune[1].misfortunes
            ]
        )
        await misfortune[0].send(
            f"{misfortune[1].role.mention}, the results of your misfortune are in!{newLine}{printout}"
        )
...
savedGuildSettings = [
    f"guilds/{d}" for d in os.listdir("guilds") if os.path.isdir(f"guilds/{d}")
]
...
async def on_command_error(ctx: discord.ext.commands.Context, error: Exception):
    ...
    if isinstance(error, discord.ext.commands.CommandNotFound):
        ...
        await ctx.send(
            f"{mention}, your decklist can not be retrieved. Check that your URL is correct."
        )
        return
    elif isinstance(error, CodFileError):
        await ctx.send(
            f"{mention}, there is an error in your .cod file. Check that the file was not changed after you saved it."
        )
        return
    elif isinstance(error, DecklistError):
        await ctx.send(
            f"{mention}, there is an error in your decklist. Check that you entered it correctly."
        )
        return
    else:
...
async def printHelp(ctx):
    await ctx.send(
        f'{ctx.author.mention}, use the command "!squirebot-help" to see the list of my commands.'
    )
...
async def confirmCommand(ctx):
    if not ctx.author.id in commandsToConfirm:
        await ctx.send(
            f"{ctx.author.mention}, there are no commands needing your confirmation."
        )
        return

    if commandsToConfirm[ctx.author.id][1] <= timeDiff(
        commandsToConfirm[ctx.author.id][0], getTime()
    ):
        await ctx.send(
            f"{ctx.author.mention}, you waited too long to confirm. If you still wish to confirm, run your prior command and then confirm."
        )
