...
class fluidRoundTournament(tournament):
    def __init__(self, name: str, hostGuildName: str, props: dict = {}):
        ...
        self.pairingsThread = threading.Thread(
            target=self._launch_pairings, args=(self.pairingWaitTime,)
        )
    ...
    def updatePairingsThreshold(self, count: int) -> None:
        ...
        if (
            self.queue.readyToPair(self.pairingsThreshold)
            and not self.pairingsThread.is_alive()
        ):
            self.pairingsThread = threading.Thread(
                target=self._launch_pairings, args=(self.pairingWaitTime,)
            )
    ...
    async def addPlayerToQueue(self, plyr: int) -> commandResponse:
        ...
        if Plyr is None:
            ...
            digest.setContent(
                f"<@{plyr}>, you are in a match that is not certified. Make sure that everyone in your last match has confirmed the result."
            )
        elif len(Plyr.decks) == 0:
            digest.setContent(
                f"<@{plyr}>, you have not submitted a deck for {self.name}. You need to do so before playing."
            )
        else:
            digest.setContent(self.queue.addPlayer(Plyr))
            self.queueActivity.append(
                (plyr, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S.%f"))
            )
            self.saveOverview()
    ...
    def _launch_pairings(self, waitTime):
        ...
        fut_pairings = asyncio.run_coroutine_threadsafe(
            self._pairQueue(waitTime), self.loop
        )
        fut_pairings.result()
    ...
    async def _pairQueue(self, waitTime: int) -> None:
        ...
        if self.queue.readyToPair(self.pairingsThreshold) and startingStr != endStr:
            self.pairingsThread = threading.Thread(
                target=self._launch_pairings, args=(0,)
            )
            self.pairingsThread.start()
    ...
    def loadOverview(self, filename: str) -> None:
        ...
        self.pairingsThreshold = int(
            fromXML(tournRoot.find("queue").attrib["threshold"])
        )
        ...
        for act in acts:
            self.queueActivity.append(
                (fromXML(act.attrib["player"]), fromXML(act.attrib["time"]))
            )
        players = tournRoot.find("queue").findall("player")
        for plyr in players:
            self.queue.addPlayer(
                self.playerReg.getPlayer(int(fromXML(plyr.attrib["name"]))),
                int(plyr.attrib["priority"]),
            )
        if (
            self.queue.readyToPair(self.pairingsThreshold)
            and not self.pairingsThread.is_alive()
