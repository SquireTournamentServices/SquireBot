...
class player:
    ...
    def __str__(self):
        ...
        digest += (
            f"Matches:{newLine}{newLine.join( [ str(mtch) for mtch in self.matches ] )}"
        )
        return digest
    ...
    def __eq__(self, other: "player"):
        ...
        if digest:
            digest &= Intersection(
                [i == j for i, j in zip(self.matches, other.matches)]
            )
        return digest
    ...
    def isActive(self) -> bool:
        ...
        return (
            self.status == PlayerStatus.SIGNED_UP
            or self.status == PlayerStatus.CHECKED_IN
        )
    ...
    def getDeckEmbed(self, a_deckname: str) -> discord.Embed:
        digest = discord.Embed(
            title=f"**{self.name}'s Deck,** **{a_deckname}**: **{self.decks[a_deckname].deckHash}**"
        )
        ...
        for card_ in self.decks[a_deckname].cards:
            ...
            fieldVals[
                "Sideboard" if isSideboard else getPrimaryType(tmpCard.types)
            ].append(card_)
    ...
    async def removeDeckAdmin(self, a_ident: str, mention) -> str:
        ...
        await self.sendMessage(
            content=f"Your deck whose name was {a_ident!r} has been removed by tournament staff."
        )
