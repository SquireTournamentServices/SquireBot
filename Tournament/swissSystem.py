...
class swissSystem(pairingSystem):
    ...
    def _isValidGroup(self, plyrs: List) -> bool:
        ...
        return Intersection(
            [A.isValidOpponent(B) for i, A in enumerate(plyrs) for B in plyrs[i + 1 :]]
        )
    ...
    def createPairings(self, standings: List, matchSize: int) -> List:
        ...
        if matchSize * len(tries[-1]) != len(standings):
            for i in range(len(standings))[::4]:
                self.savedPairings.append(
                    [standings[i], standings[i + 1], standings[i + 2], standings[i + 3]]
                )
        else:
    ...
    def getPairingsEmbed(self) -> discord.Embed:
        ...
        if len(self.savedByes) > 0:
            digest.add_field(
                name="**Byes**",
                value=", ".join([plyr.getMention() for plyr in self.savedByes]),
            )
        else:
        ...
        for i, pairing in enumerate(self.savedPairings):
            digest.add_field(
                name=f"**Table #{i+1}:**",
                value=", ".join([plyr.getMention() for plyr in pairing]),
            )
        return digest
    ...
    def exportToXML(self, indent: str) -> str:
        ...
        return "".join(
            [
                f'{indent}<player name="{p.uuid}" priority="{i}"/>\n'
