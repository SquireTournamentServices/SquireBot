...
class match:
    ...
    async def confirmResult(self, plyr: "player") -> Dict:
        ...
        if not self.isUncertified():
            digest[
                "message"
            ] = f"{plyr.getMention()}, a result for match #{self.matchNumber} has not been recorded."
        elif not plyr in self.players:
            difest[
                "message"
            ] = f"{plyr.getMention()}, you are not a player in match #{self.matchNumber}."
        elif plyr in self.droppedPlayers:
            digest[
                "message"
            ] = f"{plyr.getMention()}, you have already recorded yourself as having lost match #{self.matchNumber}. There is not need to confirm the result."
        elif plyr in self.confirmedPlayers:
            digest[
                "message"
            ] = f"{plyr.getMention()}, you have already confirmed the result of match #{self.matchNumber}."
        else:
            self.confirmedPlayers.append(plyr)
            digest[
                "message"
            ] = f"{plyr.getMention()}, your confirmation has been logged."
            if await self.confirmMatch():
                self.stopTimer = True
                digest[
                    "announcement"
                ] = f"{self.getMention()}, this match has been certified."
    ...
    async def confirmResultAdmin(self, plyr: "player", mention: str) -> Dict:
        ...
        if not self.isUncertified():
            digest[
                "message"
            ] = f"{mention}, a result for match #{self.matchNumber} has not been recorded."
        elif not plyr in self.players:
            digest[
                "message"
            ] = f"{mention}, {plyr.getMention()} is not a part of match #{self.matchNumber}."
        elif plyr in self.confirmedPlayers:
            digest[
                "message"
            ] = f"{mention}, {plyr.getMention()} has already confirmed the result of match #{self.matchNumber}."
        elif plyr in self.droppedPlayers:
            digest[
                "message"
            ] = f"{mention}, {plyr.getMention()} has already been recorded as having lost match #{self.matchNumber}. There is not need to confirm the result for them."
        else:
            self.confirmedPlayers.append(plyr)
            digest[
                "message"
            ] = f"{mention}, you have logged the confirmation of {plyr.getMention()}."
            if await self.confirmMatch():
                self.stopTimer = True
                digest[
                    "announcement"
    ...
    async def recordResult(self, plyr: "player", result: str) -> Dict:
        ...
        if self.isCertified():
            digest[
                "message"
            ] = f"Match #{self.matchNumber} is already certified. Talk to a tournament official to change the result of this match."
            return digest
        ...
        if not plyr in self.players:
            ...
            digest[
                "message"
            ] = f"{plyr.getMention()}, you have already recorded yourself as having lost match #{self.matchNumber}."
            return digest
        elif plyr == self.winner:
            digest[
                "message"
            ] = f"{plyr.getMention()}, you have already recorded yourself as the winner of match #{self.matchNumber}. Your opponents still need to confirm this result."
            return digest
        ...
        if "win" == result or "winner" == result:
            ...
            digest[
                "message"
            ] = f'{plyr.getMention()} has recorded themself as the winner of match #{self.matchNumber}. {self.getMention()}, please confirm with "!confirm-result".'
        elif "draw" == result:
            ...
            digest[
                "message"
            ] = f'{plyr.getMention()} has recorded match #{self.matchNumber} as a draw. {self.getMention()}, please confirm with "!confirm-result".'
        elif "loss" == result or "loser" == result:
            self.droppedPlayers.append(plyr)
            self.activePlayers.remove(plyr)
            digest[
                "message"
            ] = f"{plyr.getMention()}, you have been recorded as losing match #{self.matchNumber}. You will not be able to join the queue until this match is finished, but you will not need to confirm the result."
        else:
            digest[
                "message"
            ] = f'You have given an invalid result. The possible match results are "win", "draw", and "loss".'
            return digest
        ...
        if await self.confirmMatch():
            ...
            digest[
                "announcement"
            ] = f"{self.getMention()}, your match has been certified."
        else:
            self.status = MatchStatus.UNCERT

        return digest

    async def recordResultAdmin(
        self, plyr: "player", result: str, mention: str
    ) -> Dict:
        ...
        if not plyr in self.players:
            ...
            digest[
                "announcement"
            ] += " The match result was already recorded and has been overwritten."
        ...
        if "win" == result or "winner" == result:
            ...
            digest[
                "announcement"
            ] = f"{self.getMention()}, {plyr.getMention()} has been recorded as the winner of this match."
            if self.isCertified():
            ...
            digest[
                "message"
            ] = f"{mention}, {plyr.getMention()} has been recorded as the winner of match #{self.matchNumber}."
        elif "draw" == result:
            self.result = MatchResult.DRAW
            self.winner = None
            digest[
                "announcement"
            ] = f"{self.getMention()}, this match has been recorded as a draw."
            if self.isCertified():
                digest["announcement"] += " There is no need to re-confirm the result."
            else:
                self.confirmedPlayers = [plyr]
            ...
            digest[
                "message"
            ] = f"{mention}, match #{self.matchNumber} has been recorded as a draw."
        elif "loss" == result or "loser" == result:
            ...
            digest[
                "announcement"
            ] = f"{plyr.getMention()}, you have been recorded as having lost match #{self.matchNumber}. You will not need to confirm the result."
            digest[
                "message"
            ] = f"{mention}, {plyr.getMention()} has been recorded as having lost match #{self.matchNumber}."
        else:
            digest[
                "message"
            ] = f'{mention}, you have given an invalid result. The possible match results are "win", "draw", and "loss".'
            return digest
    ...
    def _getInnerXMLString(self) -> str:
        ...
        for plyr in self.droppedPlayers:
            if plyr is str:
                ...
                digest += (
                    f'\t\t<player name="{plyr.getName()}">{plyr.getUUID()}</player>\n'
                )
        digest += "\t</droppedPlayers>\n"
    ...
    def loadXML(self, a_filename: str) -> None:
        ...
        self.playerDeckVerification = str_to_bool(
            fromXML(matchRoot.find("playerDeckVerification").text)
        )
        self.gameID = int(fromXML(matchRoot.find("gameID").text))
        self.replayURL = fromXML(matchRoot.find("replayURL").text)
        self.sentOneMinWarning = str_to_bool(
            fromXML(matchRoot.find("sentWarnings").attrib["oneMin"])
        )
        self.sentFiveMinWarning = str_to_bool(
            fromXML(matchRoot.find("sentWarnings").attrib["fiveMin"])
        )
        self.sentFinalWarning = str_to_bool(
            fromXML(matchRoot.find("sentWarnings").attrib["final"])
        )
