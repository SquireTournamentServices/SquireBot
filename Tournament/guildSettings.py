...
class guildSettings:
    ...
    defaultNames = [
        "default-judge-role",
        "default-tournament-admin-role",
        "default-pairings-channel",
    ...
    def __init__(self, guild: discord.Guild):
        ...
        self.d_tournAdminRole: discord.Role = discord.utils.get(
            guild.roles, name="Tournament Admin"
        )

        self.d_pairingsChannel: discord.channel = discord.utils.get(
            guild.channels, name="pairings"
        )
        self.d_standingsChannel: discord.channel = discord.utils.get(
            guild.channels, name="Standings"
        )
        self.d_VCCatergory: discord.CategoryChannel = discord.utils.get(
            guild.categories, name="Matches"
        )
    ...
    async def configureGuild(self, author: discord.Member) -> str:
        ...
        if self.d_tournAdminRole is None:
            self.d_tournAdminRole = await self.guild.create_role(
                name="Tournament Admin"
            )
        if self.d_pairingsChannel is None:
            self.d_pairingsChannel = await self.guild.create_text_channel(
                name="pairings"
            )
        if self.d_VCCatergory is None:
    ...
    def updateDefaults(self, defaults: Dict) -> str:
        ...
        filteredDefaults = filterProperties(
            self.guild,
            {
                prop: defaults[prop]
                for prop in defaults
                if not (prop in self.defaultNames)
            },
        )
        ...
        if "default-judge-role" in defaults:
            tmp = self.guild.get_role(
                int(get_ID_from_mention(defaults["default-judge-role"]))
            )
            if not tmp is None:
                self.d_judgeRole = tmp
                filteredDefaults["successes"]["default-judge-role"] = defaults[
                    "default-judge-role"
                ]
            else:
                filteredDefaults["failures"]["default-judge-role"] = defaults[
                    "default-judge-role"
                ]
        if "default-tournament-admin-role" in defaults:
            tmp = self.guild.get_role(
                int(get_ID_from_mention(defaults["default-tournament-admin-role"]))
            )
            if not tmp is None:
                self.d_tournAdminRole = tmp
                filteredDefaults["successes"][
                    "default-tournament-admin-role"
                ] = defaults["default-tournament-admin-role"]
            else:
                filteredDefaults["failures"][
                    "default-tournament-admin-role"
                ] = defaults["default-tournament-admin-role"]
        if "default-pairings-channel" in defaults:
            tmp = self.guild.get_channel(
                int(get_ID_from_mention(defaults["default-pairings-channel"]))
            )
            if not tmp is None:
                self.d_pairingsChannel = tmp
                filteredDefaults["successes"]["default-pairings-channel"] = defaults[
                    "default-pairings-channel"
                ]
            else:
                filteredDefaults["failures"]["default-pairings-channel"] = defaults[
                    "default-pairings-channel"
                ]
        if "default-standings-channel" in defaults:
            tmp = self.guild.get_channel(
                int(get_ID_from_mention(defaults["default-standings-channel"]))
            )
            if not tmp is None:
                self.d_standingsChannel = tmp
                filteredDefaults["successes"]["default-standings-channel"] = defaults[
                    "default-standings-channel"
                ]
            else:
                filteredDefaults["failures"]["default-standings-channel"] = defaults[
                    "default-standings-channel"
                ]
        if "default-vc-category" in defaults:
            tmp = self.guild.get_channel(
                int(get_ID_from_mention(defaults["default-vc-category"]))
            )
            if not tmp is None:
                self.d_VCCatergory = tmp
                filteredDefaults["successes"]["default-vc-category"] = defaults[
                    "default-vc-category"
                ]
            else:
                filteredDefaults["failures"]["default-vc-category"] = defaults[
                    "default-vc-category"
                ]
        if "tournament-type" in defaults:
            if defaults["tournament-type"] in tournamentTypes:
                self.d_tournType = defaults["tournament-type"]
                filteredDefaults["successes"]["tournament-type"] = defaults[
                    "tournament-type"
                ]
            else:
                filteredDefaults["failures"]["tournament-type"] = defaults[
                    "tournament-type"
                ]
        ...
        if len(filteredDefaults["successes"]) == 0:
            ...
            digest += "\n\t- ".join(
                [f"{d}: {defaults[d]}" for d in filteredDefaults["successes"]]
            )
        if len(filteredDefaults["failures"]) > 0:
            ...
            digest += "\n\t- ".join(
                [f"{d}: {defaults[d]}" for d in filteredDefaults["failures"]]
            )
        if len(filteredDefaults["undefined"]) > 0:
            ...
            digest += "\n\t- ".join(
                [f"{d}: {defaults[d]}" for d in filteredDefaults["undefined"]]
            )
    ...
    def save(self, dirName: str = "") -> None:
        ...
        if not (
            os.path.isdir(f"{dirName}/currentTournaments/")
            and os.path.exists(f"{dirName}/currentTournaments/")
        ):
            os.mkdir(f"{dirName}/currentTournaments/")

        if not (
            os.path.isdir(f"{dirName}/closedTournaments/")
            and os.path.exists(f"{dirName}/closedTournaments/")
        ):
    ...
    def saveSettings(self, filename: str = "") -> None:
        ...
        digest += "<settings>\n"
        digest += (
            f'\t<guild name="{toSafeXML(self.guild.name)}" id="{self.guild.id}"/>\n'
        )
        if self.d_judgeRole is None:
    ...
    def loadSettings(self, filename: str) -> None:
        ...
        try:
            self.d_judgeRole = self.guild.get_role(
                int(fromXML(root.find("judgeRole").attrib["id"]))
            )
        except ValueError:
            pass
        try:
            self.d_tournAdminRole = self.guild.get_role(
                int(fromXML(root.find("tournAdminRole").attrib["id"]))
            )
        except ValueError:
            pass
        try:
            self.d_pairingsChannel = self.guild.get_channel(
                int(fromXML(root.find("pairingsChannel").attrib["id"]))
            )
        except ValueError:
            pass
        try:
            self.d_standingsChannel = self.guild.get_channel(
                int(fromXML(root.find("standingsChannel").attrib["id"]))
            )
        except ValueError:
            pass
        try:
            self.d_VCCatergory = self.guild.get_channel(
                int(fromXML(root.find("VCCatergory").attrib["id"]))
            )
        except ValueError:
            pass
        ...
        self.updateDefaults(
            {
                fromXML(prop): fromXML(root.find("properties").attrib[fromXML(prop)])
                for prop in root.find("properties").attrib
            }
        )
    ...
    async def loadTournaments(self, dirName: str) -> None:
        tournDirs: list = [
            f"{dirName}/{tournName}/"
            for tournName in os.listdir(dirName)
            if os.path.isdir(f"{dirName}/{tournName}/")
        ]
        for tournDir in tournDirs:
            try:
                tourn = tournamentSelector(
                    f"{tournDir}/tournamentType.xml",
                    tournDir.split("/")[-2],
                    self.guild.name,
                    {},
                )
