...
commandSnippets[
    "admin-register"
] = "- admin-register : Registers a player for a tournament"
...
async def adminAddPlayer(ctx, tourn=None, plyr=None):
    ...
    if tourn is None or plyr is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament and player in order to add someone to a tournament."
        )
        return
    ...
    if member is None:
        ...
        commandsToConfirm[ctx.author.id] = (
            getTime(),
            30,
            tournObj.addDummyPlayer(plyr, mention),
        )
        await ctx.send(
            f"{mention}, there is not a player named {plyr!r}. You can add a dummy player in with that name. Is that what you want to do (!yes/!no)?"
        )
        return
...
commandSnippets[
    "admin-add-deck"
] = "- admin-add-deck : Registers a deck for a player in a tournament"
...
async def adminAddDeck(ctx, tourn=None, plyr=None, ident=None):
    ...
    if tourn is None or plyr is None or ident is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament, a player, a deck identifier, and a decklist in order to add a deck for someone."
        )
        return
    ...
    decklist = re.sub(
        "^[^A-Za-z0-9\w\/]+", "", ctx.message.content[index:].replace('"', "")
    ).strip()
...
commandSnippets[
    "admin-remove-deck"
] = "- admin-remove-deck : Removes a deck for a player in a tournament"
...
async def adminRemoveDeck(ctx, tourn=None, plyr=None, ident=None):
    ...
    commandsToConfirm[ctx.author.id] = (
        getTime(),
        30,
        tournObj.removeDeckAdmin(member.id, deckName, mention),
    )
    await ctx.send(
        f"{mention}, in order to remove the deck {deckName} from {member.mention}, confirmation is needed. Are you sure you want to remove the deck (!yes/!no)?"
    )


commandSnippets[
    "list-players"
] = "- list-players : Lists all player (or the number of players) in a tournament "
...
async def adminListPlayers(ctx, tourn=None, num=None):
    ...
    if tourn is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament in order to list the players."
        )
        return
    ...
    if len(tournObj.playerReg.getPlayers()) == 0:
        await ctx.send(
            f"{mention}, there are no players registered for the tournament {tourn}."
        )
        return
    ...
    if num == "n" or num == "num" or num == "number":
        await ctx.send(
            f"{mention}, there are {len(playerNames)} active players in {tourn}."
        )
        return
    else:
        newLine = "\n\t- "
        await ctx.send(
            f"{mention}, the following are all active players registered for {tourn}:"
        )
...
commandSnippets[
    "player-profile"
] = "- player-profile : Lists out a player's profile, including decks names, matches, and status"
...
commandSnippets[
    "admin-match-result"
] = "- admin-match-result : Record the result of a match for a player"
...
async def adminMatchResult(ctx, tourn=None, plyr=None, mtch=None, result=None):
    ...
    try:
        ...
        await ctx.send(
            f"{mention}, you did not provide a match number. Please specify a match number as a number."
        )
        return
...
] = "- admin-confirm-result : Confirms the result of a match on a player's behalf"
...
commandSnippets[
    "give-time-extension"
] = "- give-time-extension : Give a match more time in their match"
...
async def giveTimeExtension(ctx, tourn=None, mtch=None, t=None):
    ...
    if tourn is None or mtch is None or t is None:
        await ctx.send(
            f"{mention}, you did not provide enough information. You need to specify a tournament, a match number, and an amount of time."
        )
        return
    ...
    try:
        ...
        await ctx.send(
            f"{mention}, you did not provide a match number correctly. Please specify a match number using digits."
        )
        return
    ...
    try:
        ...
        await ctx.send(
            f"{mention}, you did not provide an amount of time correctly. Please specify a match number using digits."
        )
        return

    if t < 1:
        await ctx.send(
            f"{mention}, you can not give time extension of less than one minute in length."
        )
        return
...
commandSnippets[
    "deck-check"
] = "- deck-check : See all the decks of the players in a match."
...
async def matchStatus(ctx, tourn=None, mtch=None):
    ...
    for plyr in Match.activePlayers:
        if len(plyr.decks) == 0:
            await ctx.send(
                f"{mention}, {plyr.getMention()} does not have a deck registered."
            )
        for dck in plyr.decks:
            await ctx.send(
                f"{mention}, here is the decklist for {dck} of {plyr.getMention()}:",
                embed=plyr.getDeckEmbed(dck),
            )
...
async def matchStatus(ctx, tourn=None, mtch=None):
    ...
    if mtch > len(tournObj.matches):
        await ctx.send(
            f"{mention}, the match number that you specified is greater than the number of matches. Double check the match number."
        )
        return
