...
testFiles = [
    ...
    if os.path.isfile(f"testCases/{f}")
]
tests: dict = {}
queue: list = []
...
async def runTests(msg, *args):
    ...
    if sendWarning:
        await msg.channel.send(
            content="One or more of the tests you specified aren't valid test names. Use the !view-tests command to see what tests are available."
        )
    await sendCommand(msg)
